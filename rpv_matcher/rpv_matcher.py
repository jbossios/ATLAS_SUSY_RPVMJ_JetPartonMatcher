...
class RPVParton(ROOT.TLorentzVector):
  def __init__(self, *args):
    ...
    if len(args) == 4:
      self.SetPtEtaPhiE(args[0], args[1], args[2], args[3])
    self.__quark_barcode = -999 # barcode of last quark in chain corresponding to this FSR
...
class RPVMatcher():
  ...
  def __get_parton_info_and_decorate_jet(self, partons, jet, case, info_dict):
    if 'jet_matched_barcode' in info_dict: # when jets are matched to partons using FT decisions
      ...
      if case == 'Parton': self.__matched_partons.append(jet_matched_barcode)
      ...
      if case == 'Parton': self.__matched_partons.append(info_dict['matched_parton_barcode'])
    ...
    for findex in current_matched_fsrs:
      if current_matched_fsrs[findex]['quark_barcode'] == info_dict['matched_parton_barcode']: # found another matched FSR from same last quark from gluino
        ...
        if pt_priority and partons[findex].Pt() > partons[info_dict['matched_parton_index']].Pt():
          ...
          self.__remove_decoration(self.__jets[current_matched_fsrs[findex]['jet_index']])
  ...
  def __matcher_recompute_deltar_values(self, partons, is_fsr, dr_cut):
    ...
    for jet_index, jet in enumerate(self.__jets): # loop over jets
      ...
      for parton_index, parton in enumerate(partons): # loop over partons
        ...
        if barcode in self.__matched_partons: # skip matched parton/FSR
          if not is_fsr: continue # always skip matched last-quark in gluino decay chain
          elif not self.__properties['MatchFSRsFromMatchedGluinoDecays']: continue # skip FSRs associated to matched gluino decay (unless asked not to)
    ...
    for jet_index, jet in enumerate(self.__jets): # loop over jets
      ...
      if jet_matched_barcode != -1: # make sure jet matches a parton/FSR
        if is_fsr:
          ...
          if quark_barcode not in self.__matched_partons: # make sure this quark is not already matched
  ...
  def __return_jets(self) -> [RPVJet]:
    ...
    return self.__jets if not self.__properties['ReturnOnlyMatched'] else [jet for jet in self.__jets if jet.is_matched()]

  def __match_use_deltar_values_from_ft(self) -> [RPVJet]:
    """ Match jets to partons using FT decisions """
    self.__log.debug('Jets will be matched to partons{} using DeltaR values from FactoryTools'.format(' and FSRs' if self.__fsrs else ''))
    self.__log.debug('Will return {} jets'.format('only matched' if self.__properties['ReturnOnlyMatched'] else 'all'))
  ...
  def __match_recompute_deltar_values(self) -> [RPVJet]:
    ...
    self.__log.debug('Jets will be matched to partons{} computing DeltaR values using a maximum DeltaR value of {}'.format(' and FSRs' if self.__fsrs else '', self.__properties['DeltaRcut']))
    self.__log.debug('Will return {} jets'.format('only matched' if self.__properties['ReturnOnlyMatched'] else 'all'))
    self.__matcher_recompute_deltar_values(self.__partons, False, self.__properties['DeltaRcut'])
    if self.__fsrs and self.__get_n_matched_jets() < 6:
      self.__matcher_recompute_deltar_values(self.__fsrs, True, self.__properties['DeltaRcut'])
    if not self.__properties['DisableNmatchedJetProtection']:
      self.__check_n_matched_jets()
    return self.__return_jets()
  ...
  def __init__(self, **kargs):
    ...
    # Protection
    if self.__properties['MatchFSRsFromMatchedGluinoDecays'] and not self.__properties['DisableNmatchedJetProtection']:
  ...
  def match(self) -> [RPVJet]:
    ...
    self.__matched_partons = []
    self.__matched_fsrs = {}
    if self.__properties['Debug']: self.__log.setLevel('DEBUG')
    else: self.__log.setLevel('INFO')
    self.__log.debug('Calling match()')
    # Protections
    if self.__properties['MatchingCriteria'] not in self.__functions:
      self.__log.fatal(f'MatchingCriteria=={self.__properties["MatchingCriteria"]} is not supported, exiting')
      sys.exit(1)
    if self.__properties['DeltaRcut'] != self.__properties_defaults['DeltaRcut'] and 'RecomputeDeltaRvalues' not in self.__properties['MatchingCriteria']:
    ...
    if self.__fsrs: self.__check_partons(True)
    # Run appropriate matching
    return self.__functions[self.__properties['MatchingCriteria']]()
